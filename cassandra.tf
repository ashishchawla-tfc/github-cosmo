...
resource "azurerm_cosmosdb_cassandra_table" "main" {
  ...
  cassandra_keyspace_id  = "/subscriptions/${data.azurerm_client_config.main.subscription_id}/resourceGroups/${local.resource_group_name}/providers/Microsoft.DocumentDB/databaseAccounts/${azurerm_cosmosdb_account.main.name}/cassandraKeyspaces/${each.value.keyspace_name}"
